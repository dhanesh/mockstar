...
describe("RT-1.2 — type-preserving substitution", () => {
  it("preserves numbers from request.body", () => {
    ...
    const rendered = renderCompiledJson(compiled, mkCtx({ body: { amount: 5000 } })) as Record<
      string,
      unknown
    >;
  ...
  it("preserves null when the source field is null", () => {
    ...
    const rendered = renderCompiledJson(compiled, mkCtx({ body: { maybe: null } })) as Record<
      string,
      unknown
    >;
  ...
  it("id.named() resolves cross-references to the same value within one response (REV-5)", () => {
    ...
    const compiled = compileJsonValue({
      ...
      links: [
        {
          href: '/orders/{{id.named("order_id", "", 17, "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ")}}',
          rel: "self",
        },
        {
          href: '/orders/{{id.named("order_id", "", 17, "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ")}}/capture',
          rel: "capture",
        },
      ],
