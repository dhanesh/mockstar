...
import { enforceKeyPermissions, scopedCommonName, nodeExtraCaCertsMessage } from '../src/features/proxy/ca.ts';
...
describe('enforceKeyPermissions (S1 / G7)', () => {
  async function makeKeyFile(mode: number): Promise<{ caRoot: string; rootCertPem: string; rootKeyPem: string }> {
