...
async function generateSelfSigned(host: string): Promise<{
  ...
  if (result.status !== 0) {
    throw new Error(`openssl req failed (exit ${result.status}): ${result.stderr?.toString("utf8") ?? ""}`);
  }
...
const SKIP_REASON =
  "openssl not available on PATH — full TLS handshake test requires openssl to generate a valid self-signed cert";
...
describe("TLS server integration (T1 / RT-2 / RT-3 / G3)", () => {
  ...
  beforeAll(async () => {
    ...
    scratchDir = dir;
    const snapshot = makeSnapshot([{ host: "api.razorpay.com", tenant: "razorpay" }], certPem, keyPem);
  ...
  it("snapshot swap + handler dispatch is atomic (captured snapshot wins)", async () => {
    ...
    const next = makeSnapshot([{ host: "api.razorpay.com", tenant: "razorpay" }], certPem, keyPem);
