...
import { compileScenarioRules, evaluateScenarios, type ScenarioAttrs } from '../src/core/scenarios/evaluator.ts';
...
function rule(id: string, when: ScenarioRuleT['when'], response: ScenarioRuleT['response'] = { status: 404 }): ScenarioRuleT {
...
describe('StringMatch predicates (T6)', () => {
  ...
  it('contains predicate', () => {
    ...
    expect(evaluateScenarios(compiled, makeAttrs({ params: { msg: 'payment_fail_retry' } })).match?.id).toBe('r');
  });
...
describe('O1: scenario miss reason', () => {
  ...
  it('returns no missReason when predicate attribute present but value does not match', () => {
    const compiled = compileScenarioRules([rule('r', { params: { lastName: 'Test' } })]);
    const { match, scenarioMissReason } = evaluateScenarios(compiled, makeAttrs({ params: { lastName: 'Smith' } }));
    expect(match).toBeNull();
...
describe('multi-dimension predicates', () => {
  it('all dimensions must match', () => {
    const compiled = compileScenarioRules([
      rule('r', { params: { id: 'x' }, query: { fail: 'true' } }),
    ]);
    // Only params match, query does not
    expect(evaluateScenarios(compiled, makeAttrs({
      params: { id: 'x' },
