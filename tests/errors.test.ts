...
const captureLogger = (): { logger: ReturnType<typeof createLogger>; lines: string[] } => {
  const lines: string[] = [];
  const logger = createLogger({ stdout: (l) => lines.push(l), stderr: (l) => lines.push(l), deterministic: true });
  return { logger, lines };
};
...
describe('invokeWithBoundary (tier 1 of TN2)', () => {
  it('returns handler response on success', async () => {
    ...
    const ctx = { var: { requestId: 'r1', tenant: 't1' } } as unknown as Parameters<typeof invokeWithBoundary>[0];
