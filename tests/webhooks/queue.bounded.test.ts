...
// `unknown` parameterisation on the capture's resolver — Promise.withResolvers's
// resolve type is contravariant in its parameter, so callers can pass any
// {resolve} that accepts a strict subtype. We accept the broadest shape here.
// biome-ignore lint/suspicious/noExplicitAny: contravariance — see comment above
type AnyCapture = { onTerminal: { resolve: (value: any) => void } };
...
describe("BoundedRetryQueue", () => {
  ...
  test("on cap overflow drops OLDEST waiting entry, not newest (O1)", async () => {
    ...
    q.enqueue(
      makeDelivery("d2", async () => ({ httpStatus: 200, durationUs: 1, resolvedUrl: "x" }), captures[1]!),
    ); // waiting
    q.enqueue(
      makeDelivery("d3", async () => ({ httpStatus: 200, durationUs: 1, resolvedUrl: "x" }), captures[2]!),
    ); // evicts d2
    q.enqueue(
      makeDelivery("d4", async () => ({ httpStatus: 200, durationUs: 1, resolvedUrl: "x" }), captures[3]!),
    ); // evicts d3
  ...
  test("drops resolve via onTerminal, do NOT reject (TN2)", async () => {
    ...
    q.enqueue(
      makeDelivery("dropme", async () => ({ httpStatus: 200, durationUs: 1, resolvedUrl: "x" }), captureDrop),
    );
