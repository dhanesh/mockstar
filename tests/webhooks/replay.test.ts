...
describe('O4 / INT-2 — admin replay endpoint re-enqueues', () => {
  test('replay returns 404 for unknown deliveryId', async () => {
    ...
    const server = createServer({ holder, registry: emptyRegistry, deterministic: true, installCrashHandlers: false });

    const response = await server.hono.fetch(new Request(
      'http://localhost/__admin/tenants/default/webhooks/no-such-id/replay',
  ...
  test('replay returns 410 when the webhook spec has been removed from snapshot', async () => {
    ...
    await server.hono.fetch(new Request('http://localhost/orders', { method: 'POST', body: '{}', headers: { 'content-type': 'application/json' } }));
    ...
    const response = await server.hono.fetch(new Request(
      `http://localhost/__admin/tenants/default/webhooks/${deliveryId}/replay`,
      { method: 'POST', headers: { authorization: `Bearer ${ADMIN_TOKEN}` } },
    ));
  ...
  test('replay returns 202 with new deliveryId when spec still exists', async () => {
    ...
    // Replay.
    const response = await server.hono.fetch(new Request(
      `http://localhost/__admin/tenants/default/webhooks/${originalDeliveryId}/replay`,
      { method: 'POST', headers: { authorization: `Bearer ${ADMIN_TOKEN}` } },
    ));
    expect(response.status).toBe(202);
    const body = await response.json() as { replayQueued: boolean; originalDeliveryId: string; newDeliveryId: string };
  ...
  test('replay endpoint requires tenant-scope auth', async () => {
    ...
    const server = createServer({ holder, registry: emptyRegistry, deterministic: true, installCrashHandlers: false });
    const response = await server.hono.fetch(new Request(
      'http://localhost/__admin/tenants/default/webhooks/anything/replay',
      { method: 'POST' },
    ));
