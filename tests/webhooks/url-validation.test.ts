...
import { validateUpstreamUrl, isPrivateHost } from '../../src/features/url-validator.ts';
...
describe('webhook URL validation reuses validateUpstreamUrl (RT-5, T6, S2, TN4)', () => {
  ...
  test('flags are independent — allowHttp:true alone does NOT permit private (TN4)', () => {
    expect(() =>
      validateUpstreamUrl('http://10.0.0.1/hook', { allowedSchemes: ['http', 'https'] }),
    ).toThrow(/private/);
  });
  ...
  test('file:// always rejected even if allowlist tries to permit it', () => {
    expect(() =>
      validateUpstreamUrl('file:///etc/passwd', { allowedSchemes: ['file'] }),
    ).toThrow(/file/);
  });
