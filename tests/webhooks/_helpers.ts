 ...
 *
 * Returned as the strict WebhookSpec input shape so it can be assigned directly
 * to `Entry.webhooks[]` without per-callsite casts. Fields here mirror the
 * Zod-parsed defaults; `overrides` is loosely typed because tests routinely
 * supply partial-shape values for `signing`, `retry`, `expectResponse`, etc.
 */
type WebhookSpecInput = Entry extends { webhooks?: infer W }
  ? W extends ReadonlyArray<infer S>
    ? S
    : never
  : never;

export function webhookSpec(overrides: Record<string, unknown> = {}): WebhookSpecInput {
  return {
    id: "wh-test",
