...
import { ADMIN_TOKEN, makeTestServer, webhookSpec, tick, spawnReceiver } from './_helpers.ts';
...
describe('O2 — Prometheus metrics for webhook delivery cycle', () => {
  test('successful delivery increments delivery_total{outcome=success} and emits latency histogram', async () => {
    const receiver = spawnReceiver(() => new Response('{}', { status: 200, headers: { 'content-type': 'application/json' } }));
    ...
    const metricsResponse = await server.hono.fetch(new Request('http://localhost/metrics', {
