...
describe('T5 — webhook trigger surface across response.kind', () => {
  ...
  test('multiple webhooks attached to one entry — all fire', async () => {
    ...
    const entries: Entry[] = [{
      ...
      webhooks: [
        webhookSpec({ id: 'wh-1', url: receiver1.url, retry: { attempts: 1, backoff: [], jitterRatio: 0 } }),
        webhookSpec({ id: 'wh-2', url: receiver2.url, retry: { attempts: 1, backoff: [], jitterRatio: 0 } }),
      ],
    }];
    const { server } = makeTestServer({ entries });

    await server.hono.fetch(new Request('http://localhost/api/fanout', { method: 'POST', body: '{}', headers: { 'content-type': 'application/json' } }));
    await tick(300);
