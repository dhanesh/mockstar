...
import { buildHandlerRegistry, MissingHandlerError, verifyHandlerReferences } from '../src/core/handlers/index.ts';
...
describe('handler registry (RT-1)', () => {
  ...
  beforeAll(async () => {
    dir = await makeHandlersDir({
      ...
      'greet.ts':
        'export async function greet() { return new Response("hi", { status: 200 }); }\n',
    });
  });
