...
function detectCiOrContainer(): EnvHostility | null {
  ...
  if (process.env.CIRCLECI === "true") return { kind: "containerized-or-ci", detail: "CIRCLECI=true" };
  if (process.env.BUILDKITE === "true") return { kind: "containerized-or-ci", detail: "BUILDKITE=true" };
  if (process.env.container)
    return { kind: "containerized-or-ci", detail: `container=${process.env.container}` };
  return null;
}
...
async function detectMacosVpnResolver(): Promise<EnvHostility | null> {
  ...
  if (resolverCount >= 3 && hasSearchDomains) {
    return {
      kind: "vpn-resolver-override",
      detail: `scutil reports ${resolverCount} resolvers with search domains`,
    };
  }
  return null;
}
