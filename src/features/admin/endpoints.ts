...
export function adminRouter(deps: AdminDeps): Hono {
  ...
  app.get(
    "/metrics",
    adminAuthMiddleware({
      snapshot: (): ReturnType<SnapshotHolder["get"]> => deps.holder.get(),
      scope: "root",
    }),
    (ctx) => {
  ...
  app.post(
    ...
    (ctx: Context) => {
      ...
      if (!result.ok) {
        const httpStatus =
          result.code === "delivery_not_in_journal" ? 404 : result.code === "tenant_not_found" ? 404 : 410; // mock_entry_removed / webhook_spec_removed — config no longer hosts the spec
